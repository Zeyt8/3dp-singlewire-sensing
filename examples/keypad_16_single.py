...
if __name__ == "__main__":
    ...
    (
        ...
        node_kwargs={
            **default_args(generate_all_geometries)["node_kwargs"],
            ...
            "connection_node_directions": np.array([[1, 0, 0]]),
            "connection_node_length": 5,
        },
        link_kwargs={
            **default_args(generate_all_geometries)["link_kwargs"],
            "radius": [2.0] + [3.5] * (len(set_of_link_path_positions) - 1),
        },
        ...
        resistor_trace_fill_kwargs={
            **default_args(generate_all_geometries)["resistor_trace_fill_kwargs"],
            "aiming_resistance": [430e3, 103636, 94215, 85650, 77863, 70785, 64350, 58500, 53182, 48347, 43952, 39956, 36324, 33021, 30019, 27290, 430e3],
            "learning_rate": 0.1,
            "max_iterations": 300,
    ...
    # # # (100e3 in real ohm when 430e3: so, adjust rate is 0.23)
    # n_nodes = len(nodes)
    # voltage_thres = 2.5
    # wire_r, trace_r, score = optimize_resistances(
    #     n_nodes=n_nodes - 1,
    #     wire_resistance_candidates=np.arange(0.1e6, 10e6, 200e3),
    #     trace_resistance_candidates=[100e3],  # [430e3 * 0.23],
    # )
    # print(
    ...
    wire_r, trace_r, score = optimize_resistances(
        n_nodes=16,
        wire_resistance_candidates=np.arange(0.1e6, 10e6, 200e3),
        trace_resistance_candidates=[95e3],
    )
    print(f"wire: {wire_r}, trace_r: {trace_r}")
    with open(f"../models/{mesh_file_name}_wire_resistance.txt", "w") as f:
        f.write("2.3 M\n")
